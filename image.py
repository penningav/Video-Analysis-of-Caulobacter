...
"""
This module contains functions useful for basic image analysis. For every 
function, the expected input image is a numpy array.
...
from skimage import exposure, filters, img_as_float, morphology
...
def removebackground(img, bgd=np.ones(1, dtype=np.float)):
    """
    Remove defined background from image. If no background image is given, the 
    original image is returned as a float.
...
def fixflatfield(img, size=0):
    """
    Perform pseudo-flatfield correction by dividing the image by a large 
    ...
    kwargs:
        siz (int): size of averaging filter (default = 25% of the smallest 
            image dimension)
    ...
    img = img_as_float(img)
    if size == 0:
        size = np.ceil(min(img.shape) / 4)
    flt = ndimage.uniform_filter(img, size)
    return img / flt
...
def energy(img):
    """
    Calculate the image energy, defined here as the absolute value of the 
    gradients in both row and column directions of the image.
...
def img_as_uint12(img):
    """
    Convert image to 12-byte format (uint16, but with all values <4096), 
    required for some functions in skimage. Input must be positive-valued.
...
def threshold(img, use_otsu=True, level=1.):
    """
    Threshold the image at the defined level. If use_otsu = False, calculate 
    threshold based on absolute levels. Otherwise, calculate threshold using 
    the Otsu method and let the level be a fraction of this value.

    args:
        img (ndarray): input image

    kwargs:
        use_otsu (bool): specifies whether to calculate the threshold based on 
            absolute levels (False) or using the Otsu method, with the input 
            level representing a specified fraction of this value (True)
    ...
    """
    if use_otsu:
        level = level * filters.threshold_otsu(img)
    return img > level
