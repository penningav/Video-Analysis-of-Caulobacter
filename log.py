...
"""
This module is used to read/save log files metadata for each experiment. These are .txt files automatically generated by LabVIEW at the start of each experiment.
...
def read_log(input_dir):
    """
    Read the log.txt file, which contains entries in the format "Key: Value". All values are imported as strings, but some are converted to other formats for convenience, and some parameters may be added to this dictionary:
...
def read_time(input_dir, start_date, img_type='phase'):
    """
    Read times for specified image type, converting absolute to relative values. If a valid log file dictionary is given, the start time is subtracted from there; otherwise, the start time is set to the minimum value in the imported times.
...
def read_pumps(input_dir, log_dict):
    """
    Read in metadata about pump start/stop times, flow rates and units; also save the solution used in each pump into these results.

    args:
        input_dir (path): raw data experiment directory
        log_dict (dict): log file parameter-value pairs (from read_log), with updated fields corresponding to "Start Date" (datetime) and "Total Time" in seconds (float)
